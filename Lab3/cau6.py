...
# Spinbox đầu tiên
spinbox_frame1 = ttk.Frame(label_frame, borderwidth=6, relief='raised')  # Tạo frame với viền
spinbox_frame1.grid(column=0, row=3, sticky='W', padx=(0, 5))  # Giảm khoảng cách bên phải
spinbox1 = ttk.Spinbox(spinbox_frame1, from_=-1000, to=1000, textvariable=spin_value, width=5)
spinbox1.pack()

# Spinbox thứ hai với thuộc tính sunken
spin_value2 = tk.IntVar()  # Biến lưu giá trị Spinbox thứ hai
spinbox_frame2 = ttk.Frame(label_frame, borderwidth=6, relief='sunken')  # Tạo frame với viền sunken
spinbox_frame2.grid(column=1, row=3, sticky='W')  # Không cần khoảng cách
spinbox2 = ttk.Spinbox(spinbox_frame2, from_=-1000, to=1000, textvariable=spin_value2, width=5)
spinbox2.pack()
