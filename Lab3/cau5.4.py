...
def update_textbox(*args):
    text_box.delete(1.0, tk.END)  # Xóa nội dung Text box
    text_box.insert(tk.END, f"{spin_value.get()}")  # Chèn giá trị mới vào Text box

# Tạo hàm để thêm giá trị vào Text box khi nhấn nút
def on_click():
    current_value = spin_value.get()
    text_box.insert(tk.END, f"\n{current_value}")  # Thêm giá trị mới vào Text box mà không xóa
...
ttk.Label(label_frame, text="Select a value (-1000-1000):").grid(column=0, row=2, sticky='W')
...
spinbox = ttk.Spinbox(spinbox_frame, from_=-1000, to=1000, textvariable=spin_value, width=5)
spinbox.pack()

# Thêm nút "Click Me!"
action_button = ttk.Button(label_frame, text="Click Me!", command=on_click)  # Gọi on_click khi nhấn
